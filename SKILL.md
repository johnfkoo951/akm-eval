...
3. `references/runtime-paths.md` — 에이전트 런타임별 세션·스킬·설정 경로 레지스트리. **1단계 증거 수집 전 읽을 것** — 사용자가 Claude Code 외 런타임을 잘 쓰고 있는데 평가에 반영되지 않는 누락을 막는다.
