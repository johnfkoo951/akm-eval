...
2. (제출 시) **로컬 성적표 HTML** (`akm-report.html`) — 공식 성적표와 동일한 포맷, 브라우저로 열면 끝(서버 불필요). 파일 하나로 친구에게 공유할 수 있고, 받은 사람도 하단 링크에서 바로 자기 평가를 시작할 수 있습니다. 보드 공개를 원치 않아도 제출만 하면 발급됩니다.
